...
use Application\Form\ForgottenPasswordChangeFilter;
use Application\Form\ForgottenPasswordChangeForm;
use Zend\Json;
...
//                echo((length($addresses)));
//                $addresses = "{[1]}";//$this->getUsersTable()->getUserByEmailAddress($email_value);
...
//
    ...
    public function forgottenPasswordAction()
    {
        ...
        if ($request->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                return $this->redirect()->toRoute('application/default', array('controller' => 'registration', 'action' => 'password-send-success'));
            }
        }
        return new ViewModel(array('form' => $form));
    }
    ...
    public function forgottenPasswordChangeAction()
    {
        ...
        if($request->isGet()) {
            ...
            {
                return $this->redirect()->toRoute('application/default', array('controller' => 'registration', 'action' => 'forgotten-password-change-deactivated'));
            }
        }
...
<div class="">
    ...
    <div class="registration-button-area">
        <a href="http://' . $this->getRequest()->getServer('HTTP_HOST') . $this->url()->fromRoute('application/default', array(
...
.registration-button:hover
    {
        ...
        $message->addTo($auth->usr_email)
            ->setEncoding('UTF-8')
            ->addFrom('bok@atrakcjekrakowa.pl')
