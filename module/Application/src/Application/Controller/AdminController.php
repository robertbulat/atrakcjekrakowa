...
class AdminController extends AbstractActionController
{
    ...
    public function createAction()
    {
        ...
        if ($request->isPost()) {
			...
			if ($form->isValid()) {
				...
				return $this->redirect()->toRoute('default', array('controller' => 'admin', 'action' => 'index'));
			}			
		}
    ...
    public function updateAction()
    {
		...
		if (!$id) return $this->redirect()->toRoute('application/default', array('controller' => 'admin', 'action' => 'index'));
		$form = new UserForm();
		$request = $this->getRequest();
        if ($request->isPost()) {
			$form->setInputFilter(new UserFilter());
